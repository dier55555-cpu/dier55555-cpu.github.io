...
- `job/sudrf_tabs_rayon.md` — справочник вкладок и столбцов
- `job/sudrf_labels.py` — константы для матчинга в job/parser

Райсуд, вкладка апелляции: **`ОБЖАЛОВАНИЕ РЕШЕНИЙ, ОПРЕДЕЛЕНИЙ (ПОСТ.)`**.  
Облсуд — отдельно (названия отличаются), ждём скрины.
