...
status found → ответь клиенту **полной карточкой** из result: номер, ссылка, УИД, даты, судья, категория, стадия, все стороны, **полное** движение дела. Несколько дел → полные карточки подряд (`Дело 1 из N`). Не говори что «сайт не отвечает», если status=found.
